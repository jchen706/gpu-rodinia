...
{   
  ...
  printf("blockIDx.x: %d, threadIdx.x: %d, Size: %d, t: %d, Size-1-t: %d\n",blockIdx.x,threadIdx.x,Size,t,Size-1-t);
	...
	float b = *(a_cuda+Size*(blockDim.x*blockIdx.x+threadIdx.x+t+1)+t) / *(a_cuda+Size*t+t);
  *(m_cuda+Size*(blockDim.x*blockIdx.x+threadIdx.x+t+1)+t) = b;
  //

  printf("a_cuda index: %d, val: %f \n" ,a_cuda+Size*(blockDim.x*blockIdx.x+threadIdx.x+t+1)+t, *(a_cuda+Size*(blockDim.x*blockIdx.x+threadIdx.x+t+1)+t));
  printf("a_cuda div index: %d, val: %f \n", a_cuda+Size*t+t, *(a_cuda+Size*t+t));
  printf(" m_cuda index: %d, set to val: %f\n", m_cuda+Size*(blockDim.x*blockIdx.x+threadIdx.x+t+1)+t, b);
}
